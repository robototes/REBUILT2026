...
/**
 * Filters vision measurements around pose resets to prevent stale data from corrupting a freshly
 * reset pose.
 *
 ...
 *
 * <ol>
 *   <li>Camera captures frame at time T=1.0s, sees AprilTags, calculates robot is at (5, 3)
 *   <li>User presses reset button at T=1.05s during CommandScheduler.run()
 *   <li>drivetrain.resetPose() is called, setting pose to (0, 0) and clearing Kalman filter state
 *   <li>Vision.periodic() runs AFTER CommandScheduler.run() in the same robot loop
 *   <li>Vision processes the camera frame from T=1.0s (captured BEFORE reset)
 *   <li>addVisionMeasurement() is called with pose=(5,3) and timestamp=1.0s
 *   <li>The pose estimator applies latency compensation: "At T=1.0s you were at (5,3)"
 *   <li>This "correction" pulls the freshly reset pose back toward (5, 3) - the jump!
 * </ol>
 *
 * <p>The fix: Ignore any vision measurements with timestamps within a window around the last reset
 * time. This allows stale frames in the camera pipeline to be discarded, and gives the pose
 * estimator time to stabilize before vision corrections resume.
 */
public class VisionInjectFilter {
  /**
   * Sentinel value indicating no pose reset has occurred yet, allowing vision to work immediately
   * on startup
   */
  ...
  /**
   * Records that a pose reset has occurred at the current time. Call this when the drivetrain pose
   * is reset.
   *
  ...
  /**
   * Determines if a vision measurement should be ignored based on its timestamp relative to the
   * last pose reset.
   *
  ...
  public boolean shouldIgnore(
      Pose2d newVisionRobotPose, Pose2d currentRobotPose, double timestampSeconds) {
