...
public class DashboardConstants {
  ...
  /**
   * Properties that we write to NetworkTables (these ones are OK to change since they arent
   * associated with a Field2d object.
   */
  ...
  /**
   * Properties that we write to NetworkTables. This string is used: a) As the key for the
   * NetworkTable entry that is written to. b) As the key for the NetworkTable entry when adding a
   * Pose2d to a Field2d object on NetworkTable. c) As the ID used when calling a provider to show a
   * particular Field2d item.
   *
   * <p>It's very important that Pose2d items that will be added to a Field2d have this string match
   * the string in simgui.json. Otherwise, the formatting and border color may not be applied
   * correctly.
   */
