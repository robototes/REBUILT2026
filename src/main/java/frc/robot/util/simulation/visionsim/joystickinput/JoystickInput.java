 ...
 *
 * <p>When running in simulation the outputs of {@link #getJoystickInputs()} are additionally
 * transformed via {@link SimJoystickOrientation} so that "joystick-up" always drives toward the top
 * of the Glass field view regardless of alliance colour.
 *
 * <p>All external dependencies are injected through the constructor so this class can be
 * unit-tested without hardware or a running robot.
 */
public class JoystickInput implements JoystickInputInterface {
   ...
   * @param operatorForwardDegreesSupplier Supplies the operator forward direction in degrees (used
   *     only when {@code isSimulation} is {@code true}; may be {@code null} otherwise).
   */
   ...
   *
   * <p>In simulation mode the translation axes are additionally transformed by {@link
   * SimJoystickOrientation} so that "joystick-up" maps to "screen-up" in the Glass field view.
   *
