 ...
 * to pose estimates, and managing simulation state.
 */
public interface SimLimelightProducerInterface {
  ...
  public static record DrivetrainVisionPoseInfo(
      Pose2d pose, double timestamp, Matrix<N3, N1> estimationStdDevs, int tagCount) {}

  /** Process vision data. Should be called periodically (e.g., from robotPeriodic). */
  void periodic();

  /**
   * Update the vision simulation with the current robot pose. Should be called from
   * simulationPeriodic.
   *
  ...
  /**
   * Dynamically offsets the primary camera's simulated physical position. Does not affect what the
   * pose estimator believes — models miscalibration.
   *
