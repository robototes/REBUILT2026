...
public class GroundTruthSim implements GroundTruthSimInterface {
  ...
  /**
   * Simulated rightward pull: how far right (meters) the robot drifts for each meter of forward
   * travel. Models real-world drivetrain asymmetry.
   */
  private static final double kRightDriftMetersPerMeterForward = Inches.of(6).in(Meters);

  /**
   * Simulated clockwise rotation: how many radians the robot drifts clockwise for each meter of
   * forward travel. Models real-world drivetrain turn asymmetry.
   */
  private static final double kClockwiseRotationRadiansPerMeterForward = Degrees.of(10).in(Radians);
  ...
  /**
   * Constructs a GroundTruthSim instance with fully injectable dependencies. Package-private to
   * allow unit tests to supply deterministic collaborators without depending on vendor hardware
   * classes.
   *
  ...
  public void resetAllPosesToSelectedAutoPos(Pose2d blueAlliancePose, boolean useCorrectTeamSide) {
    ...
    Pose2d pose = shouldFlipPose ? AllianceCalc.flipFieldPose(blueAlliancePose) : blueAlliancePose;
  ...
  public void injectDriftToPoseEstimate(
      double offsetFrontBackX, double offsetLeftRightY, double rotationOffsetDegrees) {
    ...
    Pose2d driftedPose =
        currentPose.transformBy(
            new Transform2d(
                offsetFrontBackX, offsetLeftRightY, Rotation2d.fromDegrees(rotationOffsetDegrees)));
