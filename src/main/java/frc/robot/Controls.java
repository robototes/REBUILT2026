...
public class Controls {
  ...
  private void configureIndexingBindings() {
    ...
    // stop feeder motor
    indexingTestController.b().whileTrue(s.feederSubsystem.stop());

    // start spindexer motor
    indexingTestController.x().onTrue(s.spindexerSubsystem.startMotor());

    // stop spindexer motor
    indexingTestController.y().onTrue(s.spindexerSubsystem.stop());
    ...
    indexingTestController
        ...
        .whileTrue(
            Commands.startEnd(
                ...
                () -> {
                  s.feederSubsystem.stop();
                  s.spindexerSubsystem.stop();
                },
                s.feederSubsystem,
